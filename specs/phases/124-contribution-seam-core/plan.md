...
`source_function`, `evidence`) in a new `core/contracts/ze-plugin/ze_plugin/contribution.py`, built on
...
(`core/cognition/ze-memory`) with a new `provenance` field and a `signal_to_contribution()` conversion;
retrofit `OpenLoop`'s extraction path (`core/cognition/ze-worldstate`) with a `loop_to_contribution()`
...
no new dependency edge in the package graph). `core/cognition/ze-memory`, `core/cognition/ze-worldstate`,
`core/cognition/ze-correlation` each gain an import of `ze_plugin.contribution` (all three already
...
**Testing**: pytest, `asyncio_mode = "auto"`. New tests in `core/contracts/ze-plugin/tests/test_contribution.py`
(licensing table, evidence validation, rejection logging — no DB, no LLM, pure function).
Modified/added tests in `core/cognition/ze-memory/tests/dream/`, `core/cognition/ze-worldstate/tests/`,
`core/cognition/ze-correlation/tests/` mock store Protocols with `AsyncMock`; the existence-check
callables `validate_and_submit()` takes are also mocked, never a real DB.

**Target Platform**: Backend service packages (`core/contracts/ze-plugin`, `core/cognition/ze-memory`,
`core/cognition/ze-worldstate`, `core/cognition/ze-correlation`), wired transitively into `apps/ze-api`. No
new deployment unit.
...
| V. Test Discipline | New `core/contracts/ze-plugin/tests/test_contribution.py` (pure function, no DB/LLM). Modified tests in three packages mock stores and the injected existence-check callables. `make test-plugin`, `make test-memory`, `make test-worldstate`, `make test-correlation`, `make lint` all must pass. | PASS |
| VI. Explicit Persistence | One hand-written raw-SQL Alembic migration on the `zm` chain (`core/cognition/ze-memory`, the package that owns `memory_signals`) adding `provenance`. No ORM. | PASS |
