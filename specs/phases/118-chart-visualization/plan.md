...
Add a `chart` primitive (line/bar/area starter set) to `core/kernel/ze-components` and
...
**Language/Version**: Python 3.11 (backend, `core/kernel/ze-components`) / TypeScript, React 19 (frontend, `apps/ze-web`, `packages/ze-ui`)
...
- **V. Test Discipline** — PASS, planned. New tests land in `core/kernel/ze-components/tests/` (mocked, no real DB/LLM — schema/tool assertions only) and `packages/ze-ui/src/react/PrimitiveRenderer.test.tsx` (component render assertions), per research.md R6.
...
```text
core/kernel/ze-components/ze_components/
├── organisms/
...
core/kernel/ze-components/tests/
...
descriptor + tool logic goes in `core/kernel/ze-components` (already the SDUI-descriptor owner);
