...
All entities below live in `core/cognition/ze-memory`'s existing graph (`memory_entities` /
...
`Entity` dataclass (`core/cognition/ze-memory/ze_memory/types.py`):
...
**Extraction path**: `core/cognition/ze-memory/ze_memory/extractor.py`'s generic
...
Added to `core/cognition/ze-memory/ze_memory/graph/predicates.py`'s `ALL_PREDICATES`
...
`core/cognition/ze-memory/ze_memory/graph/types.py`:
...
- Reset hook: `core/ops/ze-onboarding/ze_onboarding/reset.py`'s truncation
  entry for the table.

## 6. `PriorityView`'s fourth ranked source

Not a persisted entity — a read-time signal, computed the same way the
existing three sources are (`core/arbitration/ze-priority/ze_priority/scoring.py`):

| Type | Shape |
|---|---|
| `RelationshipStalenessSource` (Protocol, new, in `core/arbitration/ze-priority`) | Structurally matches `PersonStore.list_stale_for_follow_up(stale_days, limit) -> list[StaleFollowUpNudge]` — no plugin import in `ze-priority` (research.md §3) |
