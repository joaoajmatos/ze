...
Existing monorepo layout (see root `CLAUDE.md`) — `core/automation/ze-skills/` (new package),
`core/engine/ze-core/`, `core/contracts/ze-agents/`, `core/contracts/ze-plugin/`, `apps/ze-api/`, `apps/ze-web/`.
...
- [X] T001 Create `core/automation/ze-skills/pyproject.toml` (deps: `ze-agents`, `ze-proactive`,
      `ze-logging`, `ze-data`, `httpx`, `asyncpg==0.31.0`, dev group mirrors
      `core/cognition/ze-worldstate/pyproject.toml`; `[tool.hatch.build.targets.wheel] packages =
      ["ze_skills"]`; `testpaths = ["tests"]`, `asyncio_mode = "auto"`)
- [X] T002 [P] Create `core/automation/ze-skills/ze_skills/__init__.py` (empty package marker)
- [X] T003 [P] Create `core/automation/ze-skills/tests/__init__.py` and `core/automation/ze-skills/tests/conftest.py`
      (shared fixtures: mock asyncpg pool, mock `httpx.AsyncClient`, mock embedder — mirrors
      `core/cognition/ze-worldstate/tests/conftest.py`)
...
- [X] T005 Add a `test-skills` target to the root `Makefile`, mirroring `test-worldstate`
      (`.PHONY` line + `cd core/automation/ze-skills && uv run pytest`), and list it in `make help`'s
      target list
...
- [X] T006 [P] Define `SkillStatus`, `SkillSource`, `SkillTrigger` `StrEnum`s and `Skill`,
      ...
      `core/automation/ze-skills/ze_skills/types.py` (per data-model.md; `Skill.slug` derivation helper,
...
- [X] T007 [P] Define `SkillParseError`, `SkillNotFoundError`, `InvalidSkillTransitionError` in
      `core/automation/ze-skills/ze_skills/errors.py` (subclass `ZeError` per `ze_agents/errors.py`
      hierarchy)
- [X] T008 Add `SkillUsageTrace` dataclass and `MessageTrace.skills_used: list[SkillUsageTrace]
      = field(default_factory=list)` in
      `core/engine/ze-core/ze_core/conversation/messages/types.py` (per data-model.md — existing
      fields unchanged)
- [X] T009 Create `core/automation/ze-skills/ze_skills/migrations/env.py` (Alembic env boilerplate,
      mirrors `core/cognition/ze-worldstate/ze_worldstate/migrations/env.py`)
- [X] T010 Create `core/automation/ze-skills/ze_skills/migrations/versions/zsk001_skills.py` — raw-SQL
...
- [X] T011 Register `_ZE_SKILLS_VERSIONS` constant pointing at
      `core/automation/ze-skills/ze_skills/migrations/versions` in `apps/ze-api/ze_api/migrate.py`
      (alongside `_ZE_WORLDSTATE_VERSIONS`)
- [X] T012 Implement `SkillStore` protocol and `PostgresSkillStore` in
      `core/automation/ze-skills/ze_skills/store.py` — CRUD + status-filtered list + conditional
...
- [X] T013 [P] Unit tests for `PostgresSkillStore` in
      `core/automation/ze-skills/tests/test_store.py` (mock asyncpg pool; covers create, get, list with
...
- [X] T014 [P] Unit tests for `types.py` slug derivation and `content_hash`
      stability/sensitivity in `core/automation/ze-skills/tests/test_types.py`
...
- [X] T015 [P] [US1] Unit tests for `parse_skill_md()` in
      `core/automation/ze-skills/tests/test_parser.py` — valid frontmatter, missing name/description
...
- [X] T016 [P] [US1] Unit tests for `fetch_skill_source()` in
      `core/automation/ze-skills/tests/test_importer.py` — direct `SKILL.md` URL, zip archive with
...
- [X] T017 [P] [US1] Unit tests for approve/reject transitions in
      `core/automation/ze-skills/tests/test_review.py` — pending→active, pending→rejected, 409-equivalent
...
- [X] T019 [US1] Implement `parse_skill_md()` in `core/automation/ze-skills/ze_skills/parser.py` —
...
- [X] T020 [US1] Implement `fetch_skill_source()` in `core/automation/ze-skills/ze_skills/importer.py` —
...
- [X] T021 [US1] Implement `approve_skill`, `reject_skill` in
      `core/automation/ze-skills/ze_skills/review.py` — conditional `pending_review → active` /
...
- [X] T022 [US1] Implement `import_skill()` orchestration in
      `core/automation/ze-skills/ze_skills/rest.py` — calls `fetch_skill_source()`, persists a new
...
- [X] T025 [US1] Create `build_skills_stack(shared, settings)` in
      `core/automation/ze-skills/ze_skills/bootstrap.py` — constructs `PostgresSkillStore` from the
      shared asyncpg pool
...
- [X] T027 [P] [US2] Unit tests for `SkillMatcher` in
      `core/automation/ze-skills/tests/test_matching.py` — embedding-similarity match above/below
...
- [X] T028 [P] [US2] Unit tests for tool-narrowing intersection logic in
      `core/contracts/ze-agents/tests/test_base_agent.py` (extend existing suite) — agent `tools`
...
- [X] T029 [P] [US2] Unit test for `match_skills` orchestration node in
      `core/engine/ze-core/tests/orchestration/test_skills_node.py` — populates
...
- [X] T031 [US2] Implement `SkillMatcher` in `core/automation/ze-skills/ze_skills/matching.py` —
...
- [X] T032 [US2] Add `active_skills: list[Skill]` and `skill_tool_names: list[str] | None`
      fields to `AgentContext` in `core/contracts/ze-agents/ze_agents/types.py`
- [X] T033 [US2] Create `match_skills(state, config)` node in
      `core/engine/ze-core/ze_core/orchestration/nodes/skills.py` — reads `SkillMatcher` from
...
- [X] T034 [US2] Update `record_trace` in
      `core/engine/ze-core/ze_core/orchestration/nodes/trace.py` to populate
      `MessageTrace.skills_used` from the turn's `SkillMatch` list (name, source, trigger,
      similarity)
- [X] T035 [US2] In `core/contracts/ze-agents/ze_agents/base_agent.py`: `_build_system_prompt` prepends
...
- [X] T040 [P] [US3] Unit tests for disable/enable/remove transitions in
      `core/automation/ze-skills/tests/test_review.py` (extend) — `active → disabled`, `disabled →
...
- [X] T042 [P] [US3] Unit tests for bundled-skill startup registration in
      `core/contracts/ze-agents/tests/test_bootstrap.py` (extend) — each `ZePlugin.bundled_skill_paths()`
...
- [X] T044 [US3] Implement `disable_skill`, `enable_skill`, `remove_skill` in
      `core/automation/ze-skills/ze_skills/review.py` (conditional transitions per FR-013; `remove_skill`
...
- [X] T045 [US3] Add `disable()`, `enable()`, `remove()` wrappers to
      `core/automation/ze-skills/ze_skills/rest.py`
...
- [X] T047 [P] [US3] Implement `ZePlugin.bundled_skill_paths() -> list[str]` (default `[]`) in
      `core/contracts/ze-plugin/ze_plugin/plugin.py`
- [X] T048 [US3] Import bundled-skill modules at startup in
      `core/contracts/ze-agents/ze_agents/bootstrap.py` (mirrors `_plugin_agent_module_paths`) — for
...
- [X] T052 [P] [US4] Unit tests for `refresh_skill()` content-hash comparison in
      `core/automation/ze-skills/tests/test_review.py` (extend) — changed content → `pending_review`
...
- [X] T053 [P] [US4] Unit tests for `SkillRecheckJob` in
      `core/automation/ze-skills/tests/jobs/test_recheck.py` — sweeps all imported skills regardless of
...
- [X] T055 [US4] Implement `refresh_skill()` in `core/automation/ze-skills/ze_skills/review.py` —
      ...
      `source == bundled`
- [X] T056 [US4] Add `refresh()` wrapper to `core/automation/ze-skills/ze_skills/rest.py` and
...
- [X] T058 [US4] Implement `SkillRecheckJob` (`@proactive_job`) in
      `core/automation/ze-skills/ze_skills/jobs/recheck.py` — daily cron sweep over all `source=imported`
...
- [X] T059 [US4] Add `register_proactive_jobs(scheduler, settings, stack)` to
      `core/automation/ze-skills/ze_skills/bootstrap.py` wiring `SkillRecheckJob`, honoring
...
- [x] T063 [P] Add `core/automation/ze-skills/README.md` (mirrors `core/cognition/ze-worldstate/README.md` — package
...
Task: "Unit tests for parse_skill_md() in core/automation/ze-skills/tests/test_parser.py"
Task: "Unit tests for fetch_skill_source() in core/automation/ze-skills/tests/test_importer.py"
Task: "Unit tests for approve/reject transitions in core/automation/ze-skills/tests/test_review.py"

# Then implementation, respecting the parser -> importer -> review -> rest -> routes chain:
Task: "Implement parse_skill_md() in core/automation/ze-skills/ze_skills/parser.py"
Task: "Implement fetch_skill_source() in core/automation/ze-skills/ze_skills/importer.py"
```
