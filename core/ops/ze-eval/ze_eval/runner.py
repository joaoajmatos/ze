"""
Ze Eval Runner — orchestrates scenario execution against a live Ze instance.
...
Usage (via entrypoint):
  ...
  python eval/run.py --judge --tag companion --judge-model anthropic/claude-haiku-4-5
...
from ze_eval.judge import DEFAULT_JUDGE_MODEL, judge
...
_DEFAULT_RESULTS_DIR = Path(__file__).parent.parent.parent.parent.parent / "eval" / "results"
...
async def run(args: argparse.Namespace, results_dir: Path | None = None) -> dict:
    ...
    for scenario in scenarios:
        ...
        if not error:
            routing = routing_correct(scenario, agent_used)
            tools = tools_correct(scenario, ze_result)
