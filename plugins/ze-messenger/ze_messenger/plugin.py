...
class MessengerPlugin(ZePlugin):
    ...
    def channels(self) -> list:
        ...
        self._gmail_channel = GmailChannel(
            credentials=self._google_credentials, public_url=public_url
        )
    ...
    async def startup(self, container: Any) -> None:
        ...
        automated_patterns: list[str] = self._settings.config.get("messaging", {}).get(
            "automated_senders", []
        )
    ...
    def register_proactive_jobs(
        ...
        *,
        ...
        if self._gmail_channel is not None and self._gmail_channel.supports_push:
            ...
            topic = get_api_settings().gmail_pubsub_topic or os.environ.get(
                "GMAIL_PUBSUB_TOPIC", ""
            )
            if topic:
                renewal_job = GmailWatchRenewalJob(
                    channel=self._gmail_channel, topic_name=topic
                )
                scheduler.register(
                    renewal_job, cron="0 0 * * 0"
