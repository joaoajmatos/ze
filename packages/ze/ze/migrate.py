...
Discovers migration paths from ze-core, ze, and all registered ZePlugin subclasses.
...
Usage from the shell (via Makefile):
    python -m ze.migrate                         # upgrade heads
    python -m ze.migrate upgrade [revision]
    python -m ze.migrate downgrade [revision]    # default: -1
    python -m ze.migrate current
    python -m ze.migrate history
    python -m ze.migrate heads
    python -m ze.migrate stamp <rev> [<rev>...]
"""
...
if __name__ == "__main__":
    ...
    if cmd == "upgrade":
        ...
        print(
            ...
            "Usage: python -m ze.migrate [upgrade|downgrade|current|history|heads|stamp] [args]",
            file=sys.stderr,
        )
        sys.exit(1)
