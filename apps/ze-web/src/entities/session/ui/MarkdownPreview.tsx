...
const previewComponents: Components = {
  ...
  strong: ({ children }) => <strong className="font-medium text-foreground/80">{children}</strong>,
  em: ({ children }) => <em className="italic">{children}</em>,
  code: ({ children }) => (
    <code className="rounded bg-foreground/10 px-1 py-0.5 font-mono text-[0.9em]">{children}</code>
  ),
  ...
  h1: ({ children }) => <span className="font-medium text-foreground/80">{children} </span>,
  h2: ({ children }) => <span className="font-medium text-foreground/80">{children} </span>,
  h3: ({ children }) => <span className="font-medium text-foreground/80">{children} </span>,
